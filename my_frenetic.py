...
class Synset(object):
    ...
    def near_antonyms(self):
        return self._near_antonyms
    ...
    def __str__(self):
        ...
        s += (", Instance Hypernyms: %s" % str([h._sid for h in self._inst_hypernyms]) if self._inst_hypernyms else "")
        s += (", Similar to: %s" % str([s._sid for s in self._similar_tos]) if self._similar_tos else "")
        return s + (", Near Antonyms: %s" % str([a._sid for a in self._near_antonyms]) if self._near_antonyms else "") + ")"
...
class FreNetic(object):
    ...
    # Signifier of the similar to and antonym relation in .xml dump of WOLF
    _SIMILAR_TAG_NAME = "similar_to"
    _NEAR_ANTONONYM_TAG_NAME = "near_antonym"
    ...
    def __init__(self, path=None):
        ...
        similar_to_ids, near_antonyms_ids = {}, {}
        tree = et.parse(path)
        for synset_el in tree.iter(FreNetic._SYNSET_TAG_NAME):
            ...
            for lit_el in synset_el.iter(FreNetic._LIT_TAG_NAME):
                if lit_el.text and lit_el.text.strip() != FreNetic._EMPTY_LIT:
                    ...
                    #print(span) # TODO Figure out why span gives the correct string ?
            ...
            similar_to_ids[sid], near_antonyms_ids[sid] = [], []
            for ilr_el in synset_el.iter(FreNetic._ILR_TAG_NAME):
                ...
                if ilr_el.get('type') == FreNetic._SIMILAR_TAG_NAME:
                    similar_to_ids[sid].append(ilr_el.text.strip())

                if ilr_el.get('type') == FreNetic._NEAR_ANTONONYM_TAG_NAME:
                    near_antonyms_ids[sid].append(ilr_el.text.strip())
        ...
        for sid, synset in self._synsets.items():
            ...
            synset._similar_tos = [self._synsets[hid] for hid in similar_to_ids[sid]]
            synset._near_antonyms = [self._synsets[hid] for hid in near_antonyms_ids[sid]]
    ...
    def synsets(self, lex_span, pos=None):
        ...
        if lex_span in self._lex_spans:
            ...
            return synsets
        elif str(lex_span) in self._lex_spans:
            synsets = self._lex_spans[str(lex_span)]
            if pos is not None:
                synsets = [syn for syn in synsets if syn.pos() == pos]
            return synsets
